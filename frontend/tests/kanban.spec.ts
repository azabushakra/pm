...
test("persists new cards after reload", async ({ page }) => {
  await login(page);
  const firstColumn = page.locator('[data-testid^="column-"]').first();
...
test("chat prompt shows response and board update", async ({ page }) => {
  ...
  await expect(page.getByText("Renamed Backlog to Ideas.")).toBeVisible();
  await expect(page.locator('input[aria-label="Column title"]').first()).toHaveValue("Ideas");
});
